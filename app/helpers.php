 ...
 * @return mixed
 */
function storeCnicImage($user, $image)
{
    $mediaId = $user->addMedia($image)
        ->toMediaCollection(User::COLLECTION_PROFILE_CNIC, config('app.media_disc'))->id;
 ...
 * @return mixed
 */
function storeTranscriptImage($user, $image)
{
    $mediaId = $user->addMedia($image)
        ->toMediaCollection(User::COLLECTION_PROFILE_TRANSCRIPT, config('app.media_disc'))->id;
 ...
 * @return mixed
 */
function storeOfferLetterImage($user, $image)
{
    $mediaId = $user->addMedia($image)
        ->toMediaCollection(User::COLLECTION_PROFILE_OFFER_LETTER, config('app.media_disc'))->id;
 ...
 */
function getDoctorsDesignations()
{
    /** @var DoctorDepartment $doctorDesignations */
    $doctorDesignations = DoctorDesignation::all()->pluck('name', 'id')->sort();
...
{
    /** @var DoctorCategory $doctorCategories */
    $doctorCategories = DoctorCategory::all()->pluck('name', 'id')->sort();

    return $doctorCategories;
}

/**
 * @return DoctorCategoryNo
 */
function getDoctorsCategoryNo()
{
    /** @var DoctorCategory Unique Id $doctorCategories */
    $category_id = DoctorCategory::select('category_id')->orderBy('category_id','desc')->take(1)->value('category_id');
    $category = ($category_id)? $category_id + 1 : 121001;
    return $category;
}

/**
 * @return DoctorDesignationNo
 */
function getDoctorsDesignationNo()
{
    /** @var DoctorsDesignation Unique Id $doctorDesignations */
    $designation_id = DoctorDesignation::select('designation_id')->orderBy('designation_id','desc')->take(1)->value('designation_id');
    $designation = ($designation_id)? $designation_id + 1 : 121001;
    return $designation;
}

function getRadiologyCateNo()
{
    /** @var DoctorsDesignation Unique Id $doctorDesignations */
...
function getMenuLinks($menu)
{
    ...
    if ($menu == User::MAIN_DOCTOR) {
        ...
        $subMenus = ['Doctors', 'Departments', 'Designations', 'Schedules', 'Prescriptions'];
    }
    //diagnosis module
