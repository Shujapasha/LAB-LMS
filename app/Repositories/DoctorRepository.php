...
class DoctorRepository extends BaseRepository
{
    ...
    public function store($input, $mail = true)
    {
        try {
            ...
            if (isset($input['cnicimage']) && ! empty($input['cnicimage'])) {
                $mediaIds = storeCnicImage($user, $input['cnicimage']);
            }

            if (isset($input['transcript']) && ! empty($input['transcript'])) {
                $mediaIdss = storeTranscriptImage($user, $input['transcript']);
            }

            if (isset($input['offerletter']) && ! empty($input['offerletter'])) {
                $mediaIdsss = storeOfferLetterImage($user, $input['offerletter']);
            }

            $doctor = Doctor::create([
                'user_id'              => $user->id,
                'doctor_department_id' => $input['doctor_department_id'],
                'doctor_designation_id' => $input['doctor_designation_id'],
                'doctor_category_id' => $input['doctor_category_id'],
    ...
    public function getDoctorAssociatedData($doctorId)
    {
        ...
        // dd($data['doctorData']->user->offer_letter_url);
