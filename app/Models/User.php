...
class User extends Authenticatable implements HasMedia, MustVerifyEmail
{
    ...
    const COLLECTION_PROFILE_CNIC = 'profile_cnic';
    const COLLECTION_PROFILE_TRANSCRIPT = 'profile_transcript';
    const COLLECTION_PROFILE_OFFER_LETTER = 'profile_offer_letter';
    ...
    const CNIC_COLUMN = 'cnic_url';
    const TRANSCRIPT_COLUMN = 'transcript_url';
    const OFFER_LETTER_COLUMN = 'offer_letter_url';
     ...
     * @return mixed
     */
    public function getCnicUrlAttribute()
    {
        /** @var Media $media */
        $media = $this->getMedia(self::COLLECTION_PROFILE_CNIC)->first();
        if (! empty($media)) {
            return $media->getFullUrl();
        }
    ...
    {
        /** @var Media $media */
        $media = $this->getMedia(self::COLLECTION_PROFILE_OFFER_LETTER)->first();
        if (! empty($media)) {
            return $media->getFullUrl();
        }
    ...
    {
        /** @var Media $media */
        $media = $this->getMedia(self::COLLECTION_PROFILE_TRANSCRIPT)->first();
        if (! empty($media)) {
            return $media->getFullUrl();
        }
