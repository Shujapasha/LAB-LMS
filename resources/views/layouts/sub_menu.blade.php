...
<div class="menu menu-lg-rounded menu-column menu-lg-row menu-state-bg menu-title-gray-700 menu-state-title-primary menu-state-icon-primary menu-state-bullet-primary menu-arrow-gray-400 fw-bold my-5 my-lg-0 align-items-stretch {{ (!Request::is('doctors*','doctor-departments*','schedules*','prescriptions*')) ? 'd-none' : '' }}"
    ...
    @endrole
    @role('Admin')
    @module('Doctor Designations',$modules)
    <div class="menu-item me-lg-1 {{ Request::is('doctor-designations*') ? 'show' : '' }}">
        <?php
        $style = 'style=';
        $background = 'white-space:';
        ?>
        <a class="menu-link py-3"
           href="{{ route('doctor-designations.index') }}">
            <span class="menu-title" {{$style}}"{{$background}}nowrap">{{ __('messages.doctor_designations') }}</span>
        </a>
    </div>
    @endmodule
    @endrole
    @role('Admin')
    @module('Doctor Categories',$modules)
    <div class="menu-item me-lg-1 {{ Request::is('doctor-categories*') ? 'show' : '' }}">
        <?php
        $style = 'style=';
        $background = 'white-space:';
        ?>
        <a class="menu-link py-3"
           href="{{ route('doctor-categories.index') }}">
            <span class="menu-title" {{$style}}"{{$background}}nowrap">{{ __('messages.doctor_categories') }}</span>
        </a>
    </div>
    @endmodule
    @endrole
